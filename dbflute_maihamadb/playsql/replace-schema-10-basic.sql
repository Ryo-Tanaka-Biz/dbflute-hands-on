...
本当は良くないが、Exampleなのでひとまず暗号化していない。',
    ...
    REMINDER_USE_COUNT INTEGER NOT NULL COMMENT 'リマインダ回数: パスワードを忘れた際のリマインダ機能の利用回数。このカラムから忘れっぽい人を分析する',
    REGISTER_DATETIME DATETIME NOT NULL,
    REGISTER_USER VARCHAR(200) NOT NULL,
    UPDATE_DATETIME DATETIME NOT NULL,
    UPDATE_USER VARCHAR(200) NOT NULL,
...
create table MEMBER_SERVICE(
    MEMBER_SERVICE_ID INTEGER AUTO_INCREMENT NOT NULL COMMENT '会員サービスID: 会員サービスを識別するID。連番として基本的に自動採番される。',
